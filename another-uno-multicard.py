# Try to run uno on multiple cards
# Yet not successful
...
def main(args):
    ...
    train_sampler = DistributedSampler(train_dataset, num_replicas=world_size, shuffle=True)
    test_sampler = DistributedSampler(test_dataset, num_replicas=world_size, shuffle=False)
...
def train(args, model, sk, loss_per_head, train_dataloader, optimizer, scheduler, logger):
    ...
    for batch in bar:
        ...
        model.module.normalize_prototypes()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--nproc_per_node", default=2, type=int, help="num of nodes")
    ...
    main(args)
    #  python uno.py --dataset CIFAR100 --gpus 0 --max_epochs 200 --batch_size 256 --num_labeled_classes 50
