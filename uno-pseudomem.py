# Continual Learning setting with labelled data replayed
...
import wandb
from data.get_dataset import get_pseudo_replay_datasets
from model.MultiHeadResNet import MultiHeadResNetWithDirectFeats
import os
from argparse import ArgumentParser
from datetime import datetime
from tqdm import tqdm
from utils.eval import split_cluster_acc_v1, split_cluster_acc_v3
import numpy as np
...
def main(args):
    ...
    dataset = get_pseudo_replay_datasets(args.dataset, args)
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        test_results = test(args, model, test_dataloader, best_head, prefix="test", iteration_num=epoch)
        train_results = test(args, model, val_dataloader, best_head, prefix="train", iteration_num=epoch)
...
@torch.no_grad()
def test(args, model, val_dataloader, best_head, prefix, iteration_num):
    model.eval()
    all_labels = None
    all_preds = None
    with torch.no_grad():
    ...
    for head in range(args.num_heads):
        if prefix == "test":
            _res = split_cluster_acc_v3(all_labels, all_preds[head], num_seen=args.num_labeled_classes, head_num=head, iteration_num=iteration_num)
        elif prefix == "train":
...
def train(args, model, sk, loss_per_head, train_dataloader, optimizer, scheduler, logger):
    ...
    for batch in bar:
        ...
        views_lab, labels_lab, _, tmp_views_unlab, labels_unlab, _ = batch
        views_lab = views_lab.cuda()
        views_lab_tmp = [views_lab, views_lab, views_lab, views_lab]
        views_lab = views_lab_tmp
        views_unlab = [v.cuda() for v in tmp_views_unlab]
        images = [[vl, vu] for vl, vu in zip(views_lab, views_unlab)]
        ...
        with amp_cm:
            ...
            # In this case, this is equivalent to the setting below:
            # We train two small heads for each head, and the performance is an average of the two heads
...
if __name__ == "__main__":
    ...
    parser.add_argument("--val_ratio", type=float, default=10, help="the percentage of val unlabelled data")
    parser.add_argument("--label_val_ratio", type=float, default=50, help="the percentage of val labelled data")
    parser.add_argument("--choice_ratio", type=float, default=100, help="the percentage of labelled data to choose")
    # typically, if we set label_val_ratio to be 50, which indicates that half of the labelled data is used for
    # validation, every 0.4 of the choice_ratio corresponds with 1 labelled data to replay in train, and val set
    ...
    args.model_save_dir = os.path.join(args.checkpoint_dir, "discover",
                                       ...
                                       f"unlabeled-{args.num_unlabeled_classes}-choice_ratio-{args.choice_ratio}.pth")
    args.exp_name = "-".join(["discover", args.arch, args.dataset, args.comment,
                              str(args.num_labeled_classes), str(args.num_unlabeled_classes), str(int(args.choice_ratio))])
    main(args)

    # python uno-pseudomem.py --dataset CIFAR100Con --gpus 0 --max_epochs 200 --batch_size 512 --num_labeled_classes 50 --num_unlabeled_classes 50 --pretrained
    # checkpoints/pretrained/supervised-CIFAR100-resnet18-labeled-50-unlabeled-50-ratio-100.pth--num_heads 4 --comment
    # UNO-pseudo --offline --multicrop --amp --overcluster_factor 1 --choice_ratio 100 --label_val_ratio 0 --save-model
