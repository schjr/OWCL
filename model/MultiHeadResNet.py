...
class MultiHeadResNetWithDirectFeats(nn.Module):
    def __init__(
        self,
        arch,
        low_res,
    ...
    def forward(self, views, is_train):

        if is_train:
            # A mixture of feats and images
            if isinstance(views, list):
                original_feats = [f[0] for f in views]
                cal_feats = [self.encoder(view[1]) for view in views]
                feats = [torch.cat([fl, fu]) for fl, fu in zip(original_feats, cal_feats)]
                out = [self.forward_heads(f) for f in feats]
                out_dict = {"feats": torch.stack(feats)}
                for key in out[0].keys():
                    out_dict[key] = torch.stack([o[key] for o in out])
