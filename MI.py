...
def main(args):
    ...
    mi_criterion = MI(temperature=args.mi_temperature, is_logit=True)

    start_epoch = 0
    if args.resume:
        # Load checkpoint.
        print('==> Resuming from checkpoint..')
        checkpoint = torch.load('model_path/discover-resnet18-CIFAR100-MI-50-50-50_133checkpoint.pth.tar')
        start_epoch = checkpoint['epoch']
        model.load_state_dict(checkpoint['state_dict'])
        optimizer.load_state_dict(checkpoint['optimizer'])
    # train
    for epoch in range(start_epoch, args.max_epochs):
        loss_per_head = train(args, model, sk, loss_per_head, train_dataloader, optimizer, scheduler, mi_criterion, wandb)
...
def train(args, model, sk, loss_per_head, train_dataloader, optimizer, scheduler, mi_criterion, logger):
    model.train()
    ...
    for batch in bar:
        ...
        with amp_cm:
            ...
            # mi loss
            loss_mi = mi_criterion(outputs["logits_lab"][:, :, mask_lab], logits[:, :, ~mask_lab])

            loss_cluster = loss_cluster.mean()
            loss_overcluster = loss_overcluster.mean()

            loss = (loss_cluster + loss_overcluster) / 2 + loss_mi * args.mi_weight
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mi_temperature", default=0.1, type=float, help="softmax temperature")
    parser.add_argument("--mi_weight", default=1, type=float, help="softmax temperature")
    ...
    parser.add_argument("--joint_estimation_samples", default=100, type=int, help="ablation of mi")
