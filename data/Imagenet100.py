...
from config.config import imagenet_root
...
def get_imagenet100_datasets(transform_train, transform_val, num_labeled_classes,
    ...
    train_anno_file = './data/splits/ImageNet100_label_{:.0f}_{:.0f}.txt'.format(num_labeled_classes, ratio)
    val_anno_file = './data/splits/ImageNet100_val.txt'.format(num_labeled_classes, ratio)
    train_unlab_anno_file = './data/splits/ImageNet100_unlabel_{:.0f}_{:.0f}.txt'.format(num_labeled_classes,
                                                                                               ratio)
...
def generate(ratio):
    ...
    train_path = "datasets/NCD/ILSVRC2012/ImgNetTrain"
    val_path = "datasets/NCD/ILSVRC2012/ImgNetVal"
    IMG_EXTENSIONS = ['jpg', 'jpeg', 'JPG', 'JPEG']
    np.random.seed(0)
    cls_num = 50
    print("Number of ratio: {}%".format(ratio))
    fout_train_label = open('data/splits/ImageNet100_label_%d_%d.txt' % (cls_num, ratio), 'w')
    fout_train_unlabel = open('data/splits/ImageNet100_unlabel_%d_%d.txt' % (cls_num, ratio), 'w')
    ...
    for i, folder_name in enumerate(folders):
        ...
        for filename in files:
            ...
            if i < cls_num and np.random.rand() < ratio / 100:
